...
const formatDateTime = (dateStr, timeStr) => {
    ...
    const formattedDate = date.toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: '2025' });
    const formattedTime = date.toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit', hour12: true });
    return `${formattedDate} @ ${formattedTime}`;
};

export const TransportVoucher = ({ customer, voucherData }) => {
  const qrText = `GROUND TRANSPORT\nREF: ${customer.referenceNumber}\nPASSENGER: ${customer.firstName} ${customer.lastName}\n1. Airport Pickup: From ${voucherData.pickupLocation} to ${voucherData.dropoffLocation} at ${formatDateTime(voucherData.pickupDate, voucherData.pickupTime)}`;
    ...
    <div style={{ width: '900px', fontFamily: "'Inter', sans-serif", backgroundColor: 'white', display: 'flex', borderRadius: '16px', border: '1px solid #e5e7eb' }}>
      ...
      <div style={{ flexGrow: 1, padding: '24px', display: 'flex', flexDirection: 'column', fontSize: '14px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px 24px', marginTop: '20px', flexGrow: 1 }}>
          ...
          <div>
            <p style={{ fontSize: '12px', color: '#6b7280', margin: 0 }}>AIRPORT PICKUP</p>
            <p style={{ fontWeight: '600', color: '#1f2937', margin: 0 }}>{formatDateTime(voucherData.pickupDate, voucherData.pickupTime)}</p>
          </div>
          <div style={{ gridColumn: 'span 2', borderTop: '1px solid #e5e7eb', paddingTop: '12px' }}>
            ...
            <div style={{ fontSize: '13px', marginTop: '4px', color: '#374151' }}>
              <strong>1. Airport Pickup:</strong> From {voucherData.pickupLocation} to {voucherData.dropoffLocation} at {formatDateTime(voucherData.pickupDate, voucherData.pickupTime).split('@')[1]}
            </div>
          </div>
        </div>
      ...
      <div style={{ backgroundColor: '#800000', color: 'white', padding: '24px', width: '256px', flexShrink: 0, display: 'flex', flexDirection: 'column', justifyContent: 'space-between', borderRadius: '0 16px 16px 0' }}>
        <div>
          <div style={{ textAlign: 'center', paddingBottom: '12px', borderBottom: '1px solid #a83333' }}>
            <h3 style={{ fontWeight: 'bold', letterSpacing: '0.05em', margin: 0 }}>Piyam Travel</h3>
            <p style={{ fontSize: '12px', opacity: 0.8, margin: 0 }}>CUSTOMER COPY</p>
          </div>
        ...
        <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', marginTop: '16px' }}>
          <div style={{ backgroundColor: 'white', padding: '6px', borderRadius: '6px' }}>
            <QRCode value={qrText.trim()} size={110} level="H" />
          </div>
        </div>
      </div>
    </div>
