...
import { packageTypes, fileCategories, checklistTemplates } from '../data';
...
export default function AgentDashboard({ onLogout }) {
    ...
    const [keyInfo, setKeyInfo] = useState({
        agentName: 'Your Name',
        agentContact: '+447400828212',
        whatsAppNotes: 'Use VPN in Middle east for Whatsapp calls',
        groundTransportManager: 'Name @ +966...',
        customerSim: '',
        customerEmail: '',
        isEmailLocked: true
    });
    ...
    useEffect(() => {
        if (selectedCustomer) {
            setKeyInfo(selectedCustomer.keyInformation || {
                agentName: 'Your Name',
                agentContact: '+447400828212',
                whatsAppNotes: 'Use VPN in Middle east for Whatsapp calls',
                groundTransportManager: 'Name @ +966...',
    ...
    const handleFileChange = async (event) => {
        ...
        for (let i = 0; i < files.length; i++) {
            ...
            try {
                ...
                newDocuments.push({ id: Date.now() + i, category: currentUploadCategory, name: file.name, url: publicUrl, fileKey: fileKey });
            } catch (error) {
        ...
        setUploadingStatus(prev => ({ ...prev, [currentUploadCategory]: failedUploads > 0 ? `${failedUploads} file(s) failed!` : '' }));
    ...
    const handleDeleteFolder = async () => {
        ...
        try {
            ...
            deleteCustomerState(selectedCustomer.id);
    ...
    const handleExtendAccess = async (months) => {
        ...
        const customerDocRef = doc(db, "customers", selectedCustomer.id);
        await updateDoc(customerDocRef, { accessExpiresAt: newExpiryDate, lastUpdatedAt: serverTimestamp() });
    ...
    const handleAddNote = async () => {
        ...
        const note = { text: newNote.trim(), timestamp: new Date().toISOString(), agent: 'Agent Name' };
    ...
    const handleToggleEmailLock = async () => {
        const newKeyInfo = { ...keyInfo, isEmailLocked: !keyInfo.isEmailLocked };
        setKeyInfo(newKeyInfo);
        const customerDocRef = doc(db, "customers", selectedCustomer.id);
        await updateDoc(customerDocRef, { keyInformation: newKeyInfo, lastUpdatedAt: serverTimestamp() });
        updateCustomerState({ ...selectedCustomer, keyInformation: newKeyInfo, lastUpdatedAt: new Date() });
    };
    ...
    const handleQuickAdd = async (templateDoc) => {
        const newDocument = { id: Date.now(), category: templateDoc.category, name: templateDoc.name, url: templateDoc.url, fileKey: templateDoc.fileKey };
    ...
    const renderDashboard = () => (
        <div className="bg-white rounded-2xl shadow-lg p-6 md:p-8">
            ...
            <div className="flex justify-between items-center mb-6">
                <div className="relative flex-grow"><input type="text" placeholder="Search by name or reference number..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-lg focus:ring-red-800 focus:border-red-800" /><div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none"><SearchIcon /></div></div>
                <label className="ml-4 flex items-center cursor-pointer"><input type="checkbox" checked={showArchived} onChange={() => setShowArchived(!showArchived)} className="h-4 w-4 rounded border-gray-300 text-red-600 focus:ring-red-500" /><span className="ml-2 text-sm text-gray-600">Show Archived</span></label>
            </div>
        ...
        return (
            <div className="bg-white rounded-2xl shadow-lg p-6 md:p-8">
                ...
                <div className="flex flex-col md:flex-row justify-between items-start mb-6 gap-4">
                    <div>
                        <div className="flex items-center gap-4"><h1 className="text-3xl font-bold text-gray-800">{selectedCustomer.firstName} {selectedCustomer.lastName}</h1>{selectedCustomer.status === 'Completed' && <span className="text-sm font-bold text-green-800 bg-green-200 px-3 py-1 rounded-full">Completed</span>}</div>
                        <p className="text-gray-500 mt-1 font-mono">{selectedCustomer.referenceNumber}</p><p className="text-gray-600 mt-1 font-semibold">{selectedCustomer.packageType} to {selectedCustomer.destination}</p>
                    </div>
                    <div className="flex flex-wrap gap-2"><button onClick={() => setIsNotesModalOpen(true)} className="flex items-center justify-center bg-gray-200 text-gray-800 font-semibold py-2 px-4 rounded-lg hover:bg-gray-300 transition-colors"><NotesIcon />Internal Notes</button><button onClick={() => setIsVoucherModalOpen(true)} className="bg-gray-200 text-gray-800 font-semibold py-2 px-4 rounded-lg hover:bg-gray-300">Generate Access Voucher</button><button onClick={() => setIsDeleteModalOpen(true)} className="flex items-center justify-center bg-red-100 text-red-800 font-semibold py-2 px-4 rounded-lg hover:bg-red-200 transition-colors"><TrashIcon />Delete Folder</button></div>
                </div>

                <div className="mb-8 p-4 bg-yellow-50 border border-yellow-200 rounded-lg">
                    <h3 className="text-lg font-semibold text-gray-800 mb-2">Key Information</h3>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                        <div><label className="font-semibold text-gray-600">Your Agent</label><input type="text" value={keyInfo.agentName} onChange={(e) => setKeyInfo({ ...keyInfo, agentName: e.target.value })} placeholder="Agent Name" className="w-full mt-1 p-1 border rounded" /></div>
                        <div><label className="font-semibold text-gray-600">Agent Contact</label><input type="text" value={keyInfo.agentContact} onChange={(e) => setKeyInfo({ ...keyInfo, agentContact: e.target.value })} placeholder="+44..." className="w-full mt-1 p-1 border rounded" /></div>
                        <div className="md:col-span-2"><label className="font-semibold text-gray-600">WhatsApp Notes</label><input type="text" value={keyInfo.whatsAppNotes} onChange={(e) => setKeyInfo({ ...keyInfo, whatsAppNotes: e.target.value })} placeholder="e.g., Use VPN..." className="w-full mt-1 p-1 border rounded" /></div>
                        <div><label className="font-semibold text-gray-600">Ground Transport Manager</label><input type="text" value={keyInfo.groundTransportManager} onChange={(e) => setKeyInfo({ ...keyInfo, groundTransportManager: e.target.value })} placeholder="Name @ +966..." className="w-full mt-1 p-1 border rounded" /></div>
                        <div><label className="font-semibold text-gray-600">Customer Local SIM</label><p className="w-full mt-1 p-1 bg-gray-200 rounded">{keyInfo.customerSim || 'Not provided'}</p></div>
                        <div><label className="font-semibold text-gray-600">Customer Email</label><p className="w-full mt-1 p-1 bg-gray-200 rounded">{keyInfo.customerEmail || 'Not provided'}</p></div>
                    </div>
                    <div className="flex gap-4 mt-4">
                        <button onClick={handleUpdateKeyInfo} className="bg-yellow-500 text-white font-semibold py-1 px-3 rounded-lg hover:bg-yellow-600">Save Key Info</button>
                        <button onClick={handleToggleEmailLock} className={`font-semibold py-1 px-3 rounded-lg ${keyInfo.isEmailLocked ? 'bg-red-200 text-red-800' : 'bg-green-200 text-green-800'}`}>{keyInfo.isEmailLocked ? 'Unlock Email for Client' : 'Lock Client Email'}</button>
                    </div>
                </div>
